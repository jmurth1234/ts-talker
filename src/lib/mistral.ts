...
class Mistral {
  private static clients: Map<string, MistralClient> = new Map();

  public static getInstance(bot?: Bot): MistralClient {
    const apiKey = bot?.apiKey || process.env.MISTRAL_API_KEY;
    const key = `${apiKey}`;
